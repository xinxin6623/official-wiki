...
[抓取层] to-markdown CLI 批量转换 → 原始 MD + images/
    ↓
[入库层] 加 frontmatter + content-hash 去重 → raw 原文原子落入 ~/.ai-vault/wechat/raw/
...
[蒸馏层] distill:raw → concept   [索引层] index-rag:raw 切片
   ...
   → ~/.ai-vault/wechat/wiki/         → 精准/取证检索的语料源
    │                                  │
    └──────────────┬───────────────────┘
                   ↓
...
~/.ai-vault/wechat/           # 入库目标(vault 内,对齐 obwiki 布局)
├── INDEX.md                  # L0 索引层:全文 frontmatter 概览,可全量扫描
├── raw/                      # raw 原文(不可变,RAG 语料源)
│   └── <YYYY-MM-DD>__<account>__<slug>.md
└── wiki/                     # 沉淀轨 A:逻辑层
    ├── concepts/             # concept 页(200-400字,karpathy 模式)
    ├── entities/             # 实体页(学校/政策/人物)
...
### Phase 2 · 采集清单
**目标**:`sources.yaml` 作为唯一人工维护入口。

```yaml
# sources.yaml — 学校公号与待抓文章
# 低频场景下,URL 平时随手收集;需要时整批跑一次
accounts:
  - name: "XX中学"          # 公号显示名,落入 frontmatter.account
    biz: ""                  # 可选,exporter 回填时用
    articles:
...
**验收锚点**:`fetch.py --dry-run` 能正确解析清单并打印待抓 URL 数与去重后净增数。

### Phase 3 · 抓取 + 入库 orchestrator(落 raw)
**目标**:`fetch.py` 串起 抓取 → frontmatter → 去重 → 落 `raw/` → 更新 INDEX(L0 索引层)。

> 这一层只负责把原文不可变地落进 `raw/`,**不做蒸馏、不灌 RAG**——那是 Phase 4 / Phase 5 两条沉淀轨的事。raw-first。

接口契约(命令行):
```
python fetch.py            # 跑全量 sources.yaml,去重后只处理净增
python fetch.py --account "XX中学"
python fetch.py --dry-run  # 只解析+去重统计,不实抓
```

orchestrator 骨架(Claude Code 据此补全):
```python
# fetch.py — 抓取层 + 入库层(只落 raw)
# 核心职责:调用 to-markdown,加 frontmatter,content-hash 去重,原子落 raw/
...
RAW_STORE = VAULT / "raw"           # raw 原文落点(不可变,RAG 语料源)
...
def to_raw(raw_md: Path, account: str, url: str, ledger: dict) -> bool:
    ...
    fm = build_frontmatter(body, account, url, h)   # 见 §4 schema,含 distilled/rag_indexed 状态位
    slug = make_slug(raw_md.stem)
    out = RAW_STORE / f"{fm['publish_date']}__{account}__{slug}.md"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(fm_dump(fm) + "\n" + body, encoding="utf-8")
    # 图片同步搬运并改写相对路径(images/ 随文件走)
    move_images(raw_md.parent / "images", out.parent / f"{out.stem}.assets")
    ledger[h] = {"url": url, "file": out.name}
    return True

# main: load sources.yaml → for each url: fetch_one → to_raw →
#       清空 .raw → 写 ledger → 重建 INDEX.md(L0:按 account/date 分组,列 frontmatter 概览)
```

**验收锚点**:
- 重复跑两次,第二次净增 = 0(去重生效);
- `raw/` 内每篇有合法 frontmatter(含 `distilled: false` / `rag_indexed: false`),图片在同名 `.assets/` 内,MD 渲染无坏链;
- `INDEX.md`(L0 索引层)按公号 + 日期分组,可被后续 distill / 检索全量快速扫描。
...
python distill.py --account "XX中学"
...
```
入参:--since <date> | --account <name> | --range <d1..d2>
流程:
...
**验收锚点**:给定日期区间,产出结构化简报(按公号/主题分节,论断带 `[[concept]]` 与原文链接回引),对应文章 status 翻转为 `briefed`,且简报里既有 wiki 逻辑骨架、也有 RAG 原文佐证。
...
## 6. 可选模块 · exporter(仅当需要 KPI/全量回填时)

触发条件二选一:
- 要给学校做**阅读量/在看/评论的 KPI 简报**;
- 要**全量回填**某公号历史文章,而非零散收集 URL。
...
```
用法:浏览器打开 → **用自己的微信公众号扫码登录**(这是硬门槛,需有一个注册公号,个人订阅号即可)→ 搜目标公号 → 导出。
...
> 默认不启用本模块。低频场景下人工维护 `sources.yaml` 更省事,也避免引入公号登录依赖。
